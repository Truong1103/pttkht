...
def test_TC02_bien_min_hop_le():
    """TC02 – Tất cả biến tại biên MIN → Hợp lệ | Bao phủ: B1, B6, B11, B16"""
    assert ValidateDangKy(10, 2.0, 0, 1) == True


def test_TC03_bien_max_hop_le():
    """TC03 – Tất cả biến tại biên MAX → Hợp lệ | Bao phủ: B5, B10, B15, B20"""
    assert ValidateDangKy(25, 4.0, 3, 10) == True


def test_TC11_bien_minplus_tat_ca():
    """TC11 – Tất cả biến tại min+ → Hợp lệ | Bao phủ: B2, B7, B12, B17"""
    assert ValidateDangKy(11, 2.1, 1, 2) == True


def test_TC12_bien_maxminus_tat_ca():
    """TC12 – Tất cả biến tại max- → Hợp lệ | Bao phủ: B4, B9, B14, B19"""
    assert ValidateDangKy(24, 3.9, 2, 9) == True


# ── Test case không hợp lệ ngoài biên ─────────────────────────────────────

def test_TC04_tinChi_duoi_min():
    """TC04 – tinChi = 9 (< 10) → Không hợp lệ | Bao phủ: X1"""
    assert ValidateDangKy(9, 3.0, 2, 5) == False


def test_TC05_tinChi_tren_max():
    """TC05 – tinChi = 26 (> 25) → Không hợp lệ | Bao phủ: X2"""
    assert ValidateDangKy(26, 3.0, 2, 5) == False


def test_TC06_gpa_duoi_min():
    """TC06 – gpa = 1.9 (< 2.0) → Không hợp lệ | Bao phủ: X3"""
    assert ValidateDangKy(18, 1.9, 2, 5) == False


def test_TC07_gpa_tren_max():
    """TC07 – gpa = 4.1 (> 4.0) → Không hợp lệ | Bao phủ: X4"""
    assert ValidateDangKy(18, 4.1, 2, 5) == False


def test_TC08_monNo_tren_max():
    """TC08 – monNo = 4 (> 3) → Không hợp lệ | Bao phủ: X6"""
    assert ValidateDangKy(18, 3.0, 4, 5) == False


def test_TC09_hocKy_duoi_min():
    """TC09 – hocKy = 0 (< 1) → Không hợp lệ | Bao phủ: X7"""
    assert ValidateDangKy(18, 3.0, 2, 0) == False


def test_TC10_hocKy_tren_max():
    """TC10 – hocKy = 11 (> 10) → Không hợp lệ | Bao phủ: X8"""
    assert ValidateDangKy(18, 3.0, 2, 11) == False
```

### 4.3 Kết quả khi chạy pytest

```
test_TC02_bien_min_hop_le        PASSED
test_TC03_bien_max_hop_le        PASSED
test_TC11_bien_minplus_tat_ca    PASSED
test_TC12_bien_maxminus_tat_ca   PASSED
test_TC04_tinChi_duoi_min        PASSED
test_TC05_tinChi_tren_max        PASSED
test_TC06_gpa_duoi_min           PASSED
test_TC07_gpa_tren_max           PASSED
test_TC08_monNo_tren_max         PASSED
test_TC09_hocKy_duoi_min         PASSED
test_TC10_hocKy_tren_max         PASSED
